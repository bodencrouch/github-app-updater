...
try:
    ...
    def RobustLogger(*a, **k):
        return logging.getLogger('github_app_updater')
from github_app_updater._misc import ProcessorArchitecture, ensure_directory_exists, get_normalized_extension
from github_app_updater._compat import (
    get_app_dir,
    get_mac_dot_app_dir,
    is_frozen,
    remove_any,
...
from pathlib import Path  # was utility.system.path;  ChDir
from github_app_updater.downloader import FileDownloader, download_mega_file_url
from github_app_updater.restarter import RestartStrategy, Restarter, UpdateStrategy
...
class LibUpdate:
    ...
    def __init__(
        ...
        self.update_folder: str = tempfile.mkdtemp("_update", "app_updater_")
    ...
    def filename(self) -> str:
        # TODO(bodencrouch): allow customization of this in the constructor.
    ...
    def _is_downloaded(self) -> bool | None:
        ...
        # TODO(bodencrouch): Compare file hashes to ensure security
            ...
            for url in self.update_urls:
                ...
                try:
                    ...
                    if "mega.nz" in parsed_url.lower():
                        ...
                        # HACK(bodencrouch): use the latest tag for GitHub based downloads.
                        tag: str = self.latest
                        ...
                        if result:
                            ...
                            )  # TODO(bodencrouch): loop through archive names...
                            if not archive_path.is_file():
                                self.log.error(
...
class AppUpdate(LibUpdate):  # pragma: no cover
    ...
    def _win_overwrite(
        ...
        *,
        ...
        # TODO(bodencrouch): clean this up... i'm ashamed.
